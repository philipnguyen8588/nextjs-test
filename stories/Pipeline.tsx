  ...
  return (
      <div className={`container ${theme}`}>
        <div className="description-area">
          <Image src={Graphic} alt={name} className="pipeline-image" />
          <div className="pipeline-info">
            <div className="pipeline-name">{name}</div>
            <div className="pipeline-title">
              <b>{title}</b>
            </div>
            <div className="pipeline-title sub-title">
              {subTitle ? (
                  subTitle.split('\n').map((line, index) => (
                      <span key={index}>
              {line}
        ...
        <div className="status-area">
          <div className="status-header">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 50 50" fill="currentColor" width="100" height="100">
          ...
          <div className="progress-bar">
            <div className="progress-bar" title={`${(elapsedTime / totalTime) * 100}%`}>
              <div className="progress-bar-time" style={{ width: `${(elapsedTime / totalTime) * 100}%` }}></div>
            </div>
          </div>

          <div className="progress-info">
            <div className="progress-label">Status:</div>
            <div className="progress-value">{status}</div>
